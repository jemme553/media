'use client'
import Video from '@/app/video/[title]/page';
import Image from 'next/image';
import { useRouter } from "next/navigation";
import { useEffect, useState } from 'react';
import ReactPlayer from 'react-player'

export default function Upload({type, post}) {
    ...
    const uploadFiles = (file, category) => {
        if (file) {
    ...
    const Upload = async () => {
        ...
        if (data.category == 'picture') {
            if (imageFormats.includes(type.split('/')[1])) {
                const addPhoto = await fetch(`/api/saveMedia`,
                    {
                        method: 'POST',
            ...
            if (videosFormats.includes(type.split('/')[1])) {
                const addVideo = await fetch(`/api/saveMedia`,
                    {
                        method: 'POST',
                        body: JSON.stringify(data)
                    }
                ...
                if (addVideo.err) {
                    modalProps("Error", addVideo.err, false)
                } else {
                    modalProps("Success", 'the Video have been uploded successfully', true)
                }
            } else {
                modalProps("Video Format is not acceptable", 'You can choose one from those Video formats: flv, mpg, mpeg, mpv, mp4, wmv, mov, avi, flv, mkv, webm', false)
            }
        }
    }
                ...
                <><input type="checkbox" id="my_modal_1" className="modal-toggle" defaultChecked={showUploadModal} />
                    <div className="modal items-start ">
                        <div className="modal-box max-w-none w-[100%] h-[500px] flex items-center  flex-col">
                            ...
                            <form className={`flex flex-col justify-between h-[100%] ${showVideo && 'w-[100%]'}`} >
                                <div className='flex gap-2 items-center justify-center mt-3'>
                                    ...
                                    <input type='file' id="photo" className="hidden" onChange={(e) => uploadFiles(e.target.files[0], 'picture')} />
                                    <label for='video' className='text-bold flex flex-col btn btn-outline btn-info  max-w-xs' >
                                        <Image src='/video.svg' width={30} height={30} className='h-[30px] self-center' alt='user' />
                                    </label>
                                    <input type='file' id="video" className="hidden" onChange={(e) => uploadFiles(e.target.files[0], 'video')} />
                                </div>
                                {
                                    showVideo && <ReactPlayer url={previewImage} controls={true} width='100%' height='200px' />
                                }
                                {
                                    showPhoto &&
                                    <Image src={previewImage} width={50} height={50} className='h-[200px] w-[100%] self-center' alt='user' />
                                }
                ...
                <><input type="checkbox" id="my_modal_2" className="modal-toggle" defaultChecked={showModal} /><div className="modal ">
                    <div className="modal-box max-w-none flex items-center justify-center flex-col">
                        ...
                        <div className="modal-action flex items-center justify-center">
                            <label className="btn btn-primary" onClick={() => closeModal(title)}>Close</label>
                        </div>
                    </div>
                </div></>
            }
